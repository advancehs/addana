...
sys.path.append('..')
import addana
from addana.structures import Pca
...
def test_transform():
    ...
    addr_list = ["徐汇区虹漕路461号58号楼5楼", "泉州市洛江区万安塘西工业区", "福建省鼓楼区鼓楼医院",
                 "天津市"]
    # 分词模式
    transed = addana.transform(addr_list, cut=True)
    print(transed)
    assert_addr(transed)

    # 全文匹配
    transed = addana.transform(addr_list, cut=False, pos_sensitive=True)
    print(transed)
    assert_addr(transed)

    # 分词匹配 测试pos_sensitive
    transed = addana.transform(addr_list, cut=True, pos_sensitive=True)
    print(transed)
...
def test_data_from_csv():
    """测试数据加载"""
    area_map, city_map, province_area_map, province_map, latlng = addana._data_from_csv()
    print("....")
...
def test_fill_city_1():
    pca = Pca('', '', '朝阳区')
    addana._fill_city(pca, {'朝阳区': '北京市'}, True)
    assert pca.city == '北京市'


def test_jieba_extract():
    """地址全部在句子开头的情况"""
    pca, addr = addana._jieba_extract('湖北武汉复兴路111号')
    print(pca, addr)
    assert addr == '复兴路111号'
...
def test_jieba_extract2():
    """地址在句子中间的情况"""
    pca, addr = addana._jieba_extract('我家的地址是湖北武汉武昌区复兴路111号')
...
def test_jieba_extract3():
    """测试地名出现两次省名的情况"""
    pca, addr = addana._jieba_extract('我家的地址是湖北武汉武昌区复兴路111号上海市')
...
def test_full_text_extract0():
    """地址在开头"""
    pca, addr = addana._full_text_extract('湖北省武汉武昌区复兴路111号', 8)
    print(pca, addr)
    assert addr == '复兴路111号'
    assert pca.province == '湖北省'
    assert pca.province_pos == 0
...
def test_full_text_extract1():
    """地址在开头"""
    pca, addr = addana._full_text_extract('湖北武汉武昌区复兴路111号', 8)
    print(pca, addr)
    assert addr == '复兴路111号'
    assert pca.province == '湖北省'
    assert pca.province_pos == 0
...
def test_full_text_extract2():
    """地址在结尾"""
    pca, addr = addana._full_text_extract('我的家在湖北武汉武昌区', 8)
    print(pca, addr)
...
def test_full_text_extract3():
    ...
    pca, addr = addana._full_text_extract('我家的地址是湖北武汉武昌区复兴路1号', 8)
    print(pca, addr)
...
def test_full_text_extract5():
    """测试满足贪婪匹配模式"""
    pca, addr = addana._full_text_extract('湖北武汉武昌区复兴路1号', 3)
    print(pca, addr)
    assert addr == '复兴路1号'
    assert pca.province == '湖北省'
    assert pca.province_pos == 0
...
def test_full_text_extract6():
    """地址在开头 4级地址测试"""
    pca, addr = addana._full_text_extract('泉州市洛江区万安塘西工业区', 8)
    print(pca, addr)
    assert addr == '万安塘西工业区'
...
def test_handle_one_record1():
    """分词模式"""
    result1 = addana._handle_one_record("江苏淮安市人民路111号", {}, True, 0, True, True)
    print(result1)
...
def test_handle_one_record2():
    """全文模式"""
    result1 = addana._handle_one_record("江苏淮安市人民路111号", {}, False, 5, True, True)
    print(result1)
    assert result1["省"] == '江苏省'
    assert result1['市'] == '淮安市'
    assert result1['区'] == ''
...
def test_handle_one_record3():
    """省区推断市的模式"""
    result1 = addana._handle_one_record("江苏省清江浦区人民路111号", {}, False, 5, True, True)
