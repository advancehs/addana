...
def test_simple_area():
    ...
    df = addana.transform(location_str)
    print(df)


def test_city_detail():
    df_str = string_dataframe(addana.transform(['上海市浦东虹漕路461号58号楼5楼']))
    print(df_str)
    assert df_str == '上海市上海市浦东新区'


def test_city_detail_1():
    df_str = string_dataframe(addana.transform(['天津滨海新区小何小区111号']))
    print(df_str)
    assert df_str == '天津市天津市滨海新区'


def test_city_detail_2():
    df_str = string_dataframe(addana.transform(['孝感安陆小何小区111号']))
    print(df_str)
    assert df_str == '湖北省孝感市安陆市'


def test_city_detail_3():
    df_str = string_dataframe(addana.transform(['山西晋城市城区开发区怡凤小区凤巢小学对面10号楼']))
    print(df_str)
...
def test_predict_city():
    ...
    df_str = string_dataframe(addana.transform(['河北北戴河区融合小区11号']))
    print(df_str)
...
def test_predict_province():
    ...
    df_str = string_dataframe(addana.transform(['秦皇岛北戴河区融合小区11号']))
    print(df_str)
    assert df_str == '河北省秦皇岛市北戴河区'
...
def test_error_city_hainan():
    ...
    df = addana.transform(location_str, cut=False)
    print(df)
